...
int sys_proc_list(struct thread *td, struct sys_proc_list_args *uap) {
	...
	if(!uap->procs) {
		...
		*uap->num = num;
		...
		p = *allproc;
		for(int i = 0; i < num; i++) {
			memcpy(uap->procs[i].p_comm, p->p_comm,
			    sizeof(uap->procs[i].p_comm));
...
int sys_proc_rw(struct thread *td, struct sys_proc_rw_args *uap) {
	...
	if(p) {
		return proc_rw_mem(p, (void *)uap->address, uap->length,
		    uap->data, 0, uap->write);
	}
...
int sys_proc_vm_map_handle(struct proc *p, struct sys_proc_vm_map_args *args) {
	...
	if(args->maps) {
		...
		for(int i = 0; i < args->num; i++) {
			...
			memcpy(args->maps[i].name, entry->name,
			    sizeof(args->maps[i].name));
...
int sys_proc_cmd(struct thread *td, struct sys_proc_cmd_args *uap) {
	...
	case SYS_PROC_VM_MAP:
		return sys_proc_vm_map_handle(p,
		    (struct sys_proc_vm_map_args *)uap->data);
