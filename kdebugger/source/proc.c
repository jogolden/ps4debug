...
int proc_get_vm_map(struct proc *p, struct proc_vm_map_entry **entries,
	...
	info =
	    (struct proc_vm_map_entry *)malloc(num *
	    sizeof(struct proc_vm_map_entry), M_TEMP, 2);
	if(!info) {
		r = 1;
		goto error;
	}
...
int proc_rw_mem(struct proc *p, void *ptr, uint64_t size, void *data,
...
inline int proc_read_mem(struct proc *p, void *ptr, uint64_t size, void *data,
    uint64_t * n) {
	return proc_rw_mem(p, ptr, size, data, n, 0);
}

inline int proc_write_mem(struct proc *p, void *ptr, uint64_t size,
    void *data, uint64_t * n) {
...
int proc_allocate(struct proc *p, void **address, uint64_t size) {
	...
	r = vm_map_insert(map, NULL, NULL, addr, addr + size, VM_PROT_ALL,
	    VM_PROT_ALL, 0);
