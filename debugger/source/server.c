...
<<<<<<< HEAD
	if(wait4(dbgctx.pid, &status, WNOHANG, NULL)) {
		...
		uprintf("wait4 caught status 0x%X signal %i", status,
		    WSTOPSIG(status));

		struct debug_interrupt_packet *resp =
		    (struct debug_interrupt_packet *)malloc(sizeof(struct
			debug_interrupt_packet));
		if(!resp) {
			return 1;
		}
		...
		net_send_data(dbgctx.clientfd, resp,
		    sizeof(struct debug_interrupt_packet));
...
>>>>>>> cbcc9ea97c51653385f543e30e1a30abbc253df7
		...
		if(packet.magic != PACKET_MAGIC) {
			uprintf("[ps4debug] invalid packet magic %X!",
			    packet.magic);
			continue;
		}
		...
		if(length) {
			...
			uprintf("[ps4debug] recieving data length %i",
			    length);
...
void start_server() {
	...
	flag = 1;
	sceNetSetsockopt(serv, SOL_SOCKET, SO_NBIO, (char *)&flag,
	    sizeof(int));

	flag = 1;
	sceNetSetsockopt(serv, IPPROTO_TCP, TCP_NODELAY, (char *)&flag,
	    sizeof(int));
...
>>>>>>> cbcc9ea97c51653385f543e30e1a30abbc253df7
		...
		if(fd > -1 && !errno) {
			...
			flag = 1;
			sceNetSetsockopt(fd, SOL_SOCKET, SO_NBIO,
			    (char *)&flag, sizeof(int));

			flag = 1;
			sceNetSetsockopt(fd, IPPROTO_TCP, TCP_NODELAY,
			    (char *)&flag, sizeof(int));
