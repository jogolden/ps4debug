...
int connect_client() {
	...
	errno = NULL;
	int r =
	    sceNetConnect(dbgctx.clientfd, (struct sockaddr *)&server,
	    sizeof(server));
	if(r) {
		return 1;
	}
...
<<<<<<< HEAD
		...
		sys_proc_rw(dbgctx.pid, breakpoint->address,
		    &breakpoint->original, 1, 0);
	}
...
int debug_watchpt_handle(int fd, struct cmd_packet *packet) {
	...
	if(wp->enabled) {
		dbreg64.dr[wp->index] = wp->address;
		dbreg64.dr[7] |=
		    DBREG_DR7_SET(wp->index, wp->length, wp->breaktype,
		    DBREG_DR7_LOCAL_ENABLE | DBREG_DR7_GLOBAL_ENABLE);
	} else {
		dbreg64.dr[wp->index] = NULL;
		dbreg64.dr[7] |=
		    DBREG_DR7_SET(wp->index, NULL, NULL, DBREG_DR7_DISABLE);
	}

	// todo: remove this print and clean up rest of code
	uprintf("dr%i: %llX dr7: %llX", wp->index, wp->address,
	    dbreg64.dr[7]);
...
void debug_cleanup() {
	...
	if(dbgctx.pid != -1) {
		...
		if(!r) {
			for(int i = 0; i < nlwps; i++) {
				ptrace(PT_SETDBREGS, lwpids[i], &dbreg64,
				    NULL);
			}
		}
...
int debug_start_run_handle(int fd, struct cmd_packet *packet) {
    ...
    uprintf("debug_start_run_handle started...");

    net_send_status(fd, CMD_SUCCESS);

    while(1) {
        r = net_recv_data(fd, &data, sizeof(uint32_t), 0);
        
        uprintf("running %i %i", r, errno);
        ...
        if(!r) {
            ...
            if(data == CMD_DEBUG_STOP_RUN) {
                break;
            }
        }

        if(wait4(dbgctx.pid, &status, WNOHANG, NULL)) {
            uprintf("wait4 caught status %X", status);

            void *lwpinfo = malloc(0x98);
            memset(lwpinfo, NULL, 0x98);
            ...
            resp.status = status;
            memcpy(resp.tdname, (void *)(lwpinfo + 0x80), sizeof(resp.tdname));
            ...
            net_send_data(fd, &resp, sizeof(struct debug_interrupt_packet));

            // todo: i should wait for a response from the client on what to do next
...
int debug_handle(int fd, struct cmd_packet *packet) {
    switch(packet->cmd) {
        ...
        case CMD_DEBUG_STOP_RUN: {
            // this command is actually just sent to the debugger raw (recieved above in function debug_start_run_handle)
            return 1;
        }
        default:
            return 1;
